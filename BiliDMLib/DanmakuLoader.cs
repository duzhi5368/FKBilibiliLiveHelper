...
//        private object shit_lock=new object();//ReceiveMessageLoop 似乎好像大概會同時運行兩個的bug, 但是不修了, 鎖上算了
            ...
            try
            {
                ...
                if (channelId != lastroomid)
                {
                    ...
                    catch (WebException ex)
                    {
                        ...
                        {
                            // 直播间不存在（HTTP 404）
                            var msg = "该直播间疑似不存在，当前只支持使用原房间号连接";
        ...
        public async Task<bool> UserConnectAsync(int roomId, string visit_id, string cookie, Dictionary<string, string> headers, string csrf) {
            ...
            try
            {
                using (HttpResponseMessage response = await httpClient.SendAsync(HttpMethod.Post,
                                        "https://api.live.bilibili.com/room/v1/Room/room_entry_action", Postdata, headers, cookie))
                {
                    string s = await response.Content.ReadAsStringAsync();
                    UserLoginResult result;
                    if (string.IsNullOrEmpty(s))
                    {
                        this.ErrorStr = "登录失败，没有返回的数据";
                        return false;
                    }
                    result = JsonHelper.DeserializeJsonToObject<UserLoginResult>(s);
                    ...
                    {
                        this.ErrorStr = "登录失败，服务器报错：" + result.message;
                        return false;
                    }
                }
            }
